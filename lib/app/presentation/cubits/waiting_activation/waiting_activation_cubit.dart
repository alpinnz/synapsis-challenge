...
part 'waiting_activation_args.dart';
...
class WaitingActivationCubit extends Cubit<WaitingActivationState> {
  ...
  void onInitial(WaitingActivationArgs args) async {
    emit(state.copyWith(activation: state.activation.copyWith(status: Status.loading()), args: args));
    ...
    socketChannel.subscribe(channel, (res) {
      try {
        ...
        if (data.isActive) {
          emit(state.copyWith(activation: state.activation.copyWith(status: Status.success(), data: data)));
        }
